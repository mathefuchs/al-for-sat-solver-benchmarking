...
%JB: "score" and "gamma" aren't defined yet, so there is nothing to extend; score isn't even defined here, only "s", while later sections still use "score"
%JB: also, each model only predicts for one solver (there is no model covering multiple solvers), so I'm not sure whether introducing the solver as parameter makes sense
...
%TF: maybe a bit unclear. We do not manually fix anything. There is always one cluster with only time-outs if there are any.
%JB: was wondering about that as well; how do we exactly create the timeout cluster? do we only cluster with k-1, removing timeout instances, or is the timeout cluster an ordinary cluster (which might also contain on-timeout instances)?
...
%JB: maybe we should phrase this more defensively or just leave out this argument (might be dataset-dependend), (1) e.g., SATzilla 2008 does predict runtimes and (2) we could also have treid to predict normalized runtimes like Collautti et al. instead of discretizing (though prediction performance would probably still have been bad in our case)
...
%While only time-out labels (solver times out or not on a particular instance) are sufficient to significantly distinguish about \SI{87.04}{\%} of solver pairs.
...
%JB: but if we want to discriminate between solvers, what does it help to know that solver behaves similarly to other solvers?
...
% MI: Let this be the evaluation result:
...
\begin{algorithm}[htbp]
  ...
  \KwIn{Solvers $\mathcal{A}$, Instaces $\mathcal{I}$, Runtimes $r : \mathcal{A} \times \mathcal{I} \rightarrow [0, \tau]$}
  ...
  \For{$\hat{a} \in \mathcal{A}$}{
    ...
    $\left( s, \tilde{\mathcal{I}} \right) \leftarrow \operatorname{runALAlgorithm}\!\left(\mathcal{I}, \mathcal{A}', r, \hat{a}\right)$ \tcp*{Refer to Algorithm~\ref{algALBenchmark}}

    $O_{\operatorname{acc}} \leftarrow \frac{1}{|\mathcal{A}|} \sum\limits_{a \in \mathcal{A}} \left[\operatorname{sign}(s(a) - s(\hat{a})) = \operatorname{sign}(par2(a) - par2(\hat{a})) \right]$
	...
	$r' \leftarrow \sum\limits_{i \in \tilde{\mathcal{I}}} r(i, \hat{a})$\;
	$r \leftarrow \sum\limits_{i \in \mathcal{I}} r(i, \hat{a})$\;
    $O_{\operatorname{rt}} \leftarrow \frac{r'}{r}$
  ...
  \Return $\left( \bar{O}_{\operatorname{acc}},\, \bar{O}_{\operatorname{rt}} \right)$
\end{algorithm}
...
Instance families comprise instances that are derived from the same application domain, e.g., planning, cryptography, etc., and are a useful tool for both analyzing solver performance and portfolios.
...
The plotted lines represent the best performing configurations (convex hull) per top-level hyper-parameter choice, i.e., per ranking approach (Fig.~\ref{fig:annitraincolorranking}), per selection approach (Fig.~\ref{fig:annitraincolorselection}), and per stopping criterion (Fig.~\ref{fig:annitraincolorstopping}).
...
Regarding ranking approaches (Fig.~\ref{fig:annitraincolorranking}), using the $s_3$-induced ranking consistently outperforms the partially observed PAR-2 ranking for each possible value of $\delta$.
...
%TF: The lines are in any case no individual approaches as individual approaches are only points. So, any lines in this plot are a "combination" of configs. It's only a question of how to combine.
...
Random sampling is outperformed by the model-based uncertainty strategy in all cases showing the benefit of active learning.
By actively choosing the next experiment to run based on a model, we are more accurate in predicting the PAR-2 ranking of the solvers on the underlying dataset.
%JB: we should also describe performance of random sampling, which is visible in plot as well
%TF: added a few sentences
...
Figure~\ref{fig:annitrainoptgoal} also shows an interesting difference between different optimization goals.
It shows how different configurations select instances with different difficulties.
If it is, on the one hand, desired to get a \emph{rough} estimate of a solver's performance fast (low $\delta$), corresponding approaches favor selecting many \emph{easy} problem instances (fraction of instances bigger than the fraction of runtime within $\tilde{\mathcal{I}}$).
By having many observations, it is easier to build a model.
If it is, on the other hand, desired to get a \emph{good} estimate of a solver's performance in a moderate amount of time (high $\delta$), corresponding approaches favor selecting fewer \emph{difficult} problem instances (fraction of instances smaller than the fraction of runtime within $\tilde{\mathcal{I}}$).
...
It can predict a new solver's PAR-2 ranking with about \SI{92.33}{\%} accuracy ($O_{\operatorname{acc}}$), while only needing \SI{10.35}{\%} of the runtime that would be needed to evaluate the solver on the full dataset ($O_{\operatorname{rt}}$).
This trade-off provides a better way of prototyping and evaluating new SAT solvers rather than evaluating them on a manually picked subset based on domain knowledge.
The sampled instances account for about \SI{5.24}{\%} of instances indicating the preference for selecting more difficult instances as already discussed in the previous section.
The average Spearman correlation between the PAR-2 ranking and the predicted ranking scores is about \SI{0.9572}{} indicating that the differences in PAR-2 scores are quite similar to $s_3$ scores.
...
%TF: revised this section

\subsection{Instance-Family Importance}

Selection decisions of our approach also reveal the importance of different problem-instance families to our model.
Instance families comprise instances that are derived from the same application domain, e.g., planning, cryptography, etc., and are a useful tool for both analyzing solver performance and portfolios.

Figure~\ref{fig:annifinalfamilies} shows a scatter plot of problem-instance families regarding their occurrence within the dataset and the average occurrence in benchmarks created by active learning.
We use the best-performing configurations for all $\delta \in \left[0, 1\right]$ and examine the selection decisions by the active-learning approach on the SAT Competition~2022 Anniversary Track benchmark set~\cite{sat2022}.
While most families appear by the same fraction in the dataset and in active-learning-selected benchmarks, there are a few outliers that need some further discussion.
Problem instances of the families \emph{fpga}, \emph{quasigroup-completion}, and \emph{planning} are especially helpful to our model in distinguishing solvers.
Instances of these families are selected over-proportionally in comparison to their fraction in the dataset.
In contrast to that, instances of the biggest family, i.e., \emph{hardware-verification}, roughly appear with the same fraction in the dataset and in active-learning-selected benchmarks.
...
A possible explanation is that \emph{cryptography} instances are too similar, which means that a smaller fraction of them is sufficient to determine a solver's performance on all of them.
...
\begin{figure}[tb!]
  ...
  \caption{
    Scatter plot showing the \emph{importance} of different instance families to our model.
...
%A model-uncertainty-based selection approach in combination with runtime-label-prediction ranking and a ranking-convergence stopping criterion showed the consistently best results in most cases.
...
%TF: commented out
We also took a brief look at which problem-instance families are the most prevalent in selection decisions.
