...
\subsubsection{Traditional Benchmark Selection.}
Recent studies show that selecting a benchmark set in general is a difficult endeavor for multiple reasons.
Dehghani~et~al. describe how a biased benchmarking set can easily lead to fallacious interpretations~\cite{abs-2107-07002}.
Moreover, Nießl~et~al. state that benchmark-set design has many movable parts, e.g., performance measures, aggregation, and handling of missing values.
...
Benchmark selection is an important issue for solver competitions, e.g., the SAT Competition.
...
SMAC tries to find a close-to-optimal configuration for a given SAT solver on a given set of instances~\cite{HutterHL11}.
