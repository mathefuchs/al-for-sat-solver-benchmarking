...
While benchmarking a single SAT solver is not too expensive, trying out a multitude of different approaches and configurations steadily increases the evaluation cost.
The field of SAT solver benchmarking is well established.
However, traditional benchmark selection approaches do not optimize for benchmark runtime.
...
All our source code\footnote{\url{https://github.com/mathefuchs/al-for-sat-solver-benchmarking}} and and data\footnote{\url{https://github.com/mathefuchs/al-for-sat-solver-benchmarking-data}} are available on GitHub.
...
Note that our approach is \emph{not} a general improvement over all the shown approaches.
Rather, they all have slightly different goals, which do not fully align with Definition~\ref{def:new-solver-problem}.
...
In contrast to traditional passive machine-learning methods with given instance labels (cf. Figure~\ref{fig:passive}), active learning (AL) starts with no or little labeled data.
Then, it repeatedly selects interesting instances for which to acquire labels based on a gradually improving model of the agent (cf. Figure~\ref{fig:activepool}).
Figure~\ref{fig:learning} illustrates the difference between both types of learning.
AL methods are especially useful if acquiring labels is computationally expensive.
Without active-learning methods, it would not be obvious which instances to label and which not to.
On the one hand, we want to maximize the utility an instance provides to our model, i.e., rank prediction accuracy, and on the other hand, minimize the cost, i.e., predicted runtime, that is associated with its acquisition.
In that way, we obtain an accurate picture of the agent without the need of labeling every data point.
Rubens et.~al.~\cite{RubensESK15} provide a survey of recent active-learning advances.
...
We denote the new solver to be ranked by $\hat a \not\in \mathcal{A}$ and define $\mathcal{\hat A} := \mathcal{A} \cup \lbrace \hat a \rbrace$ to denote all solvers, including the new solver.
...
%
%JB: this function cannot be applied to new solvers, as the latter are not in $\mathcal{A}$ (only in $\mathcal{\hat A}$); instead of "a", could also use actual runtime as first input to \gamma, but that might lead to less straigthforward notation later in text (e.g., when defining ranking and stopping criteria)
...
%JB: slight inconsistency that we use number of clusters "k" as subscript (and the solver "a" as argument) here and at some other places, while we also use "\hat a" (i.e., the new solver) as subscript (and not as argument) in further places
%JB: at the moment, s_k(a) could not be applied to new solver anyway, since it depends on \gamma, and \gamma does not work for new solvers (see comment above)
...
A technical report by Xu et.~.al.~\cite{features} describes them in depth.
...
Our final choice is a stacking ensemble of a quadratic-discriminant analysis and a random forest.
A stacking ensemble uses a meta-model to decide which ensemble member makes the prediction on which instance.
To do so, it uses a simple decision tree.
...
So to speak, it is the maximum over the final results for each hyper-parameter choice that shares the plotted attributes.
...
A clear advantage of our approach is, though, that it indicates when to stop adding further instances.
Otherwise, it would not be clear why the shown runtime fractions would yield the best trade-off at all.
...
On a more general note, one can apply our evaluation framework to arbitrary computation-intensive problems, e.g., other $\mathcal{NP}$-complete problems than SAT, as all discussed active-learning methods are problem-agnostic.
