...
%JB: we should rename "interpretability" criterion (make it more specific), term "interpretability" can have lots of different meanings and our specific meaning is not very intuitive
...
%JB: can shortly describe benchmark selection in SAT Competition 2022 \cite{sat2022}, previous competitions (if difference procedure used; Related-Work section of Matricon et al. (2021) states that) and other competitions (e.g., those in https://www.floc2022.org/floc-olympic-games)
%JB: redundancy removal in benchmarks: Manthey and Möhle: "Better Evaluations by Analyzing Benchmark Structure"
...
%JB: we should probably cut the discussion of each approach's criteria to save space, maybe only mention the unsatisifed criteria
...
%JB: following paper might be relevant in that context, too (haven't read it): Eggensperger et al. "Efficient benchmarking of algorithm configurators via model-based surrogates"
...
%JB: should either add further incremental approaches or if there are none, should say so ("to the best of our knowledge")
...
%JB: this general motivation should go there where linearity is introduced as criterion (-> intro), not here
...
%JB: no sure if this dilemma arises here as well; dilemma is typical for searching one optimal solution in continuous space, where there is an incentive to exploit neighborhood in promising regions; however, if we find an interesting/useful (= good for assessing solver performance) instance in our approach, I'm not sure if we would continue picking similar instances ("exploting" instead of "exploring") just because we expect them to be useful as well
...
%JB: we should conduct some further research if there are other approaches using AL for SAT (or SMT, CP, etc.) benchmarking or some other task
...
%JB: as "$\operatorname{score}$" is not visibly different to rest of text, we should use some other way of highlighting
...
%JB: thus, we should reorder this section to mention argument "significant differenc"e (this paragraph) first and "deciding solver pairs" (previous paragraph, stronger argument) second (rather than giving the stronger argument first, making the second argument superfluous)
