...
Benchmarking also has many interchangeable parts, such as the performance measures used, including how measurement points are aggregated and how missing values are handled.
...
The selection criteria are instance \emph{variety} to avoid over-fitting, adapted instance \emph{hardness} (not too easy but also not too hard), and avoiding \emph{duplicate} instances.
...
The model $M$ maintains runtime transformations which we describe in the subsequent section. 
Additionally, $M$ provides a runtime-label prediction function $f : \mathcal{\hat A} \times \mathcal{I} \rightarrow \mathbb{R}$ for all solvers $\mathcal{\hat A} := \mathcal{A} \cup \lbrace \hat a \rbrace$.
The prediction function $f$ powers instance selection as described in Section~\ref{sec:main:selection}.
During model update (Algorithm~\ref{algALBenchmark},~Line~7), $f$ is trained to predict the known and acquired transformed runtimes based on instance features described in Section~\ref{sec:exdesign:data}.
...
In this section, we two instance sampling strategies, one that minimizes uncertainty and one that maximizes information-gain.
...
We define the \emph{ranking accuracy} $O_{\operatorname{acc}} \in \left[0, 1\right]$ (higher is better) by the fraction of pairs $\left(\hat{a}, a\right)$ for all $a \in \mathcal{A}$ that are decided correctly by the ranking function $\operatorname{par_2}$ (Lines~5-8).
...
Regarding ranking approaches (Fig.~\ref{fig:annitraincolorranking}), using the predicted $s_3$-induced runtime-label ranking consistently outperforms the partially observed PAR-2 ranking for each possible value of the trade-off parameter~$\delta$.
...
Regarding the stopping criteria (Fig.~\ref{fig:annitraincolorstopping}), the ranking stopping criterion performs well most consistently.
