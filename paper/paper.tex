...
For each instance $e \in \mathcal{I}$, we use a single-link hierarchial clustering algorithm to assign the runtimes in $\{ r(a, e) \mid a \in A \}$ to one of the $k$ clusters $C_1, \dots, C_k$.
...
To determine solver ranks, we define the adapted scoring function $s : A \rightarrow [1, 2 \cdot k]$ as follows.
\begin{align*}
  \operatorname{s}(a) &:= \frac{1}{|\mathcal{I}|} \sum_{e \in \mathcal{I}} \gamma'_k(a, e)
  &&
