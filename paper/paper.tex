...
Given a set of known solvers $\mathcal{A}$, instances $\mathcal{I}$ and runtimes $r$, we first create a prediction model~$\mathcal{M}$ for the new solver $\hat a$ (Line~1). 
The prediction model is used to repeatedly select an instance (Line~4) for benchmarking $\hat a$ (Line~5). 
...
Algorithm~\ref{algALBenchmark} returns the predicted score of $\hat a$ as well as the acquired runtime measurements (Line~9). 
...
% MI: This belongs to discussion at the end since this also has substantial disadvantages:
...
%\renewcommand{\thealgocf}{}
  ...
  \While(\tcp*[f]{cf. Section~\ref{sec:main:stopping}}){$\operatorname{not} \operatorname{stop}(\mathcal{M})$}{
    $e \leftarrow \operatorname{selectNextInstance}\!\left(\mathcal{M}\right)$ \tcp*{cf. Section~\ref{sec:main:sampling}}

    $t \leftarrow \operatorname{runExperiment}\!\left(\hat{a},\,  e\right)$  \tcp*{Runs $\hat{a}$ on $e$ with time-out $\tau$}

    $\operatorname{updateModel}\bigr(\mathcal{M}, (e, t)\bigl)$ \tcp*{cf. Section~\ref{sec:main:model}}
    
    \BlankLine
    $\mathcal{R} \leftarrow \mathcal{R} \cup \bigl\{ (e, t) \bigr\}$
  }
  $s \leftarrow \operatorname{predictScore}(\mathcal{M})$ \tcp*{cf. Section~\ref{sec:main:model}}
...
Rather, we filter approaches based on preliminary experimental results (cf. Section~\ref{sec:eval:prediction}) and do the main end-to-end experiments only with a subset.
...
As runtime-label prediction model, we only make use of one fixed model since an exhaustive grid search would be infeasible (cf.~Section~\ref{sec:eval:prediction}).
...
%JB: we should probably remove this section here and jump right into active learning;
% - results of classifier comparison can shortly be described in Section 4 (new subsection) or 3.2 ("Solver Model", which currently mainly describes runtime discretization); in any case, classifier should be discussed earlier
% - in current version, we already state in Section 3.2 that we use hierarchical clustering, so description of algorithm and some "performed well in preliminary experiments" could go there
...
%JB: we should state (were we introduce prediction model, i.e., either in 3.2 or a new subsection in 4) which features we use; I assume instance features + runtimes of other solvers? (else CV among solvers would not make sense for this experiment)
...
% \SI{0.9527}{} for time-out prediction
...
%JB: I always made the implicit assumption that the plots are based on learning curves, showing the progress within individual AL runs, and then multiple of these curves were combined (per top-level configuration); so the curves actually draw a line between different final results of AL runs? should be clarified somewhere
...
%JB: maybe we can drop "model-based" and use "uncertainty sampling" instead? I adapted this in Section 3, but section 4 + 5 + the plots would have to be adapted as well (else, we should revert my changes in Section 3)
...
%JB: plot says "Wilcoxon" without stopping; only minor point, but we should make naming of the components of our framework consistent
...
\begin{figure}[tbp!]
  ...
  \caption{
  	...
  	%JB: we should use "ranking accuracy" in plots, since we explicitly defined this metric earlier
...
%JB: we need to motivate choosing this \delta
...
%JB: could we (with reasonable effort) integrate the learning curve of this particular approach as a plot?
...
%JB: conclusion not straightforward from prior text; even if overall estimated score (which is an average over many instances) is good, estimate on individual instances might be quite erroneous
...
%JB: other components of framework might need tweaking as well, or this there something special about prediction model?
