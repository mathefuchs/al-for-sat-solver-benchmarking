...
Selection criteria are a instance variety to avoid over-fitting, adapted instance hardness (not too easy but also not too hard), and avoiding duplicate instances.
...
Moreover, a major shortcoming lies is the lacking \emph{scalability} with the number solvers.
...
In contrast to traditional passive machine-learning methods with given instance labels, active learning allows selecting instances for which to acquire labels.
...
% MI: skipping this elaboration, since we do not use regression
...
%The following paragraphs motivate why it makes sense to discretize runtimes.%MI: no further motivation found, commented out
...
\begin{equation}
  ...
  %MI: Why is it so important to have this zero-based?
  \label{eq:rankingeq}
\end{equation}
