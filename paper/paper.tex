...
\author{
  ...
  %JB: according to camera-ready mail, we might need to include ORCID logo and link ourselves; there is a package called "orcidlink" for that purpose
}
...
%JB: TODO: clarify that simply running a solver on all benchmarks is not a good approach in this scenario, i.e., motivate our approach better -> see arguments in rebuttal (comment of Reviewer 3)
...
%JB: TODO: clarify that competitors in table have different goals and thus our approach is not a general improvement on them, but targets a different problem (comment of Reviewer 2)
...
%JB: TODO: explain AL for non-ML audience (comment of Reviewer 2)
%JB: TODO: (re-)add figure for AL cycle or explain the latter better (comment of Reviewer 2); could also be placed in next section
...
%JB: we should harmonize use of \mathcal for A; sometimes \mathcal is used (e.g., two sentences ago), sometimes not (e.g., in previous sentence)
...
%
%JB: this function cannot be applied to new solvers, as the latter are not in $A$ (only in $\hat A$); instead of "a", could also use actual runtime as first input to \gamma, but that might lead to less straigthforward notation later in text (e.g., when defining ranking and stopping criteria)
...
%JB: TODO: explain features in more detail or clarify where to find this information (comment of Reviewer 2)
...
%JB: TODO: explain "stacking ensemble" (comment of Reviewer 2)
...
%JB: TODO: clarify how the hyperparameter analysis is conducted here, i.e., Pareto front / maximum over all other (not displayed) hyperparameters (comment of Reviewer 2)
...
%JB: TODO: explain table better, particularly implications of our approach only slightly beating random sampling (comment of Reviewer 2)
...
%JB: TODO: clarify that our approach works for any computation-intensive problem, not only NP-complete (comment of Reviewer 3)
