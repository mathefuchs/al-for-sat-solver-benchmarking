...
% (1) solving new-solver-problem once? -> apart from more runtimes to be acquired beforehand, is running our framework affected by increasing the set of existing solvers, and how does this depend on the configuration of our framework, i.e., sampling/selection/stopping? (e.g., Wilcoxon stopping criterion iterates over set of existing solvers, showing linear complexity, while fixed-subset stopping is independent from number of existing solvers)
% (2) solving new-solver-problem for each solver out of a set of solvers? -> is a rather theoretical scenario, because our algorithm itself assumes we know runtimes of all but one solver, so in practice we cannot throw in a bunch of new solvers and get a ranking over all of them
We avoid comparing pairs of solvers, which would scale quadratically.
%JB: need to discuss this; logarithmic (compare one-vs rest) / log-linear (compare each pair) effort might be sufficient if binary-search-/recursive-sorting-like procedure used
...
%JB: no sure if this dilemma arises here as well; dilemma is typical for searching one optimal solution in continuous space, where there is an incentive to exploit neighborhood in promising regions; however, if we find an interesting/useful (= good for assessing solver performance) instance in our approach, I'm not sure if we would continue picking similar instances ("exploiting" instead of "exploring") just because we expect them to be useful as well
