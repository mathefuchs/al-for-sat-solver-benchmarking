...
An algorithm configurator, however, cannot be used to \emph{rank/score} a new solver since algorithm configuration seeks to find solemnly the best performing configuration.
Also, while using a model-based selection strategy to sample configurations, instance selection is done \emph{randomly}, i.e., without building a model over instances.
...
Their approach ticks off most of our desired features.
...
First, we initialize the model $\mathcal{M}$ in Line~1.
This creates the machine-learning prediction model and also transforms runtimes as discussed in the subsequent section.
We do not need any further steps here, since we have not obtained any labeled samples yet, so cannot train anything.
The model update (Line~7) uses all acquired runtimes to train a new runtime-prediction function $f$ using common supervised machine-learning methodology.
It has also access to global instance features that assign each instance a vector in $\mathbb{R}^n$.
...
Finally, we determine the solver scores that are predicted in Line~8 by using the runtime-prediction function $f$ and the approach described in the following two sections.

\subsubsection{Runtime Transformation.}

We transform the runtimes into discrete runtime labels on a per-instance basis.
For each instance $e \in \mathcal{I}$, we use a clustering algorithm to assign the runtimes in $\bigl\{ r(a, e) \mid a \in A \bigr\}$ to one of $k$ clusters $C_1, \dots, C_k$ such that the fastest runtimes for instance $e$ are in cluster $C_1$ and the slowest are in cluster $C_{k-1}$, excluding timeouts $\tau$.
Those are always in cluster $C_{k}$.
...
In this section, we present two dynamic stopping criteria, the Wilcoxon and the ranking-stopping criterion (cf. Algorithm~\ref{algALBenchmark}, Line~1).
...
Given all the previously presented instantiations for Algorithm~\ref{algALBenchmark}, this section briefly outlines our experimental design, including our evaluation framework, used data sets, hyper-parameter choices, as well as implementation details.
...
\begin{algorithm}[tb]
  ...
  \tcp{Determine Rank Accuracy}
    ...
    \For{$a \in \mathcal{A}$}{
      \If{$\bigl(s_k(a) - s_k(\hat a)\bigr) \cdot \bigl(\operatorname{par_2}(a) - \operatorname{par_2}(\hat a)\bigr) > 0$}{
...
That means that each solver once plays the role of the new solver (Line~2).
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    The latest $x$ predictions vote on the winner on a per-instance level.
    \item
    Fallback threshold: if the difference of scores between two solvers (including the new solver $\hat{a}$) drops below \SI{0.01}, \SI{0.05}, or \SI{0.1}, use the partially observed PAR-2 ranking as a tie-breaker.
  \end{itemize}
\end{itemize}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item Runtime scaling: prefer instances with the greatest uncertainty per average runtime (over all solvers; \texttt{True} or \texttt{False}).
  \end{itemize}
...
Our end-to-end experiments follow the evaluation framework introduced in Section~\ref{sec:exdesign:eval}.
...
This means that we sample instances revealing similarities between solvers rather than differences.
This helps in building a confident model fast.
...
The range $\delta \in \left[0.2, 0.8\right]$, thereby, corresponds to the points between a runtime fraction of \SI{0.03} and \SI{0.22}.
...
Those problems share most of the relevant properties of SAT solving, i.e., there are established problem-instance features, a full benchmark run takes quite some time, and solver development traditionally requires expert knowledge to hand-select instances.
